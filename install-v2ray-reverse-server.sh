...
download_v2ray() {
  ...
  if ! curl -x "${PROXY}" -R -H 'Cache-Control: no-cache' -o "${TMP_DIRECTORY}/reverse_server_config.json" "$DOWNLOAD_CONF_LINK"; then
    echo 'error: Download failed! Please check your network or try again.'
    return 1
  fi
...
main() {
  ...
  cp -f $TMP_DIRECTORY/reverse_server_config.json ${JSON_PATH}/config.json
  "rm" -r "$TMP_DIRECTORY"
  echo "removed: $TMP_DIRECTORY"
